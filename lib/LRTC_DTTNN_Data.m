function [Lk, RSE_list]= LRTC_DTTNN_Data(D, mask, X, opt)
%% parameters
ranks=opt.ranks;
lambda=opt.lambda;
tol=opt.tol;
...
for Iter = 1: maxIter
    if mod(Iter, 50) == 0
        fprintf('DTTNN: iterations = %d  beta = %d  difference=%f time=%f', Iter, mu, errList(Iter-1),toc);
    end
    ...
    for i = 1:n3
        ...
        Uk_hat_transform(:,:,i) = my_svt(Uk_transform(:,:,i)+Y{1}(:,:,i)/mu, 1/(2*mu));
        Vk_hat_transform(:,:,i) = my_svt(Vk_transform(:,:,i)+Y{2}(:,:,i)/mu, 1/(2*mu));
        % Update Lk
